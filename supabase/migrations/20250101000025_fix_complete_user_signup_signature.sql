-- ==============================================================================
-- FIX: Update fn_complete_user_signup function signature
-- ==============================================================================
-- This migration fixes the fn_complete_user_signup function to accept text
-- instead of uuid for p_product_id, allowing both UUID and client_id values
-- ==============================================================================

-- Drop the old function signature if it exists (with uuid for p_product_id)
DROP FUNCTION IF EXISTS public.fn_complete_user_signup(uuid, uuid, uuid, text, boolean);

-- Create the new function that accepts text (UUID or client_id) for p_product_id
CREATE OR REPLACE FUNCTION public.fn_complete_user_signup(
  p_user_id uuid,
  p_product_id text,  -- Changed from uuid to text to support client_id
  p_plan_id uuid,
...
BEGIN
  ...
  -- 7. Create subscription (using v_actual_product_id)
...
GRANT EXECUTE ON FUNCTION public.fn_complete_user_signup(uuid, text, uuid, text, boolean) TO authenticated;

COMMENT ON FUNCTION public.fn_complete_user_signup(uuid, text, uuid, text, boolean) IS 
  'Complete user signup flow without payment providers: creates organization, assigns user as Owner, creates trial/active subscription (payment emulated), and assigns default product role. Accepts product_id as UUID or client_id (text).';
